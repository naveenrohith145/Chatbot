...
import socket
import psutil  # Import psutil for network interface detection

from utils.agents.agent import GraphvizTool, MermaidTool
from database import databaseFunctionality as dbf
from utils.tools.context import contextTool as contextTool
from utils.tools.manualTest import manualTestCase as manualTestCase
...
# Initialize counters based on existing data using the database module
dbf.initialize_counters(FEEDBACK_JSON, IMAGE_JSON)

app = Flask(__name__)

print(f"welcome to fireGPT version {version}")
...
# In-memory storage for conversation history and file context
conversation_histories = {}
user_file_contexts = {}

# Original vector database loaded from the directory
main_faiss_db = None
...
def create_contextual_prompt_with_model(query, context, message_history, model, is_code_query):
    ...
    if use_history and message_history.messages:
        ...
        # Rest of your history formatting code...
...
# Setup retrievers with hybrid search capabilities
...
# Caching for document retrieval
...
# ENHANCED MEMORY-AWARE RETRIEVAL WITH MODEL SELECTION
...
# Enhanced retrieve_and_answer_with_best_retriever function - KEPT FOR BACKWARD COMPATIBILITY
def retrieve_and_answer_with_best_retriever(vector_db, query, conversation_history):
    ...
    try:
        ...
        # If there's an error, fall back to basic retrieval method
        return retrieve_and_answer(vector_db, query, conversation_history)

# Updated retrieve_and_answer function to properly use context
def retrieve_and_answer(vector_db, query, conversation_history):
    # Configure retriever with more neighbors for comprehensive context
    retriever = vector_db.as_retriever(
        search_type="similarity", 
    ...
    try:
        ...
        for doc in docs:
            if "source" in doc.metadata:
                sources.add(os.path.basename(doc.metadata["source"]))  # Extract just filename
        ...
        print(f"Error retrieving or answering: {e}")
...
# Function to validate and improve response quality
...
def fix_diagram_syntax_with_llm(original_code, error_message, original_query):
    ...
    try:
        ...
        fix_prompt = config["fix_prompt_template"].format(
...
def process_and_display_diagram(query, result):
    """
    Original diagram processing function - now calls the enhanced version
    """
...
# Function to store documents in a vector database
def store_vector_db(documents, embeddings, folder_path, index_name):
...
# Enhanced image handling with more detailed descriptions
...
def handle_document_upload(file, user_id):
    ...
    if file_extension == '.doc':
        ...
        documents = extract_text_from_doc(document_path)
    else:
        # Handle other document types
        loader = get_loader(document_path, file.filename)
        if loader:
            documents = loader.load()
        else:
            # Fallback for unsupported file types
            from langchain.schema.document import Document
            documents = [Document(page_content=f"Unsupported file type: {file_extension}", metadata={"source": document_path})]
    ...
    user_db = store_vector_db(chunked_docs, embed, user_db_path, index_name)
...
# Enhanced image upload with better contextual information
...
# Enhanced Excel upload with better data extraction
...
# ==================================================
# MANUAL TEST CASE FUNCTIONS - Add this in the functions section
# ==================================================
...
def chat():
    ...
    chat_id = data.get('chat_id', None)  # For continuing existing chats
    
    # Process diagram requests
    is_diagram_request, modified_query = process_diagram_request(query)
    if is_diagram_request:
        print(f"Diagram request detected. Original: '{query}', Modified: '{modified_query}'")
    ...
    if isinstance(conversation_histories[user_id], dict):
        ...
        # Convert to dict format with session_id
    ...
    # Process test case generation requests - MOVED HERE after session_id is defined
    ...
    try:
        # Check if user has a file context
        if user_id in user_file_contexts:
            context = user_file_contexts[user_id]
            
            if context["context_type"] == "image":
                # For images, we need to reprocess with the new query
                result = image_handling(context["file_path"], query)
                
                # Get or generate image_id and image_hash
                ...
                # For files with vector databases, use memory-aware retrieval with model selection
            ...
            # Use main database with memory-aware retrieval AND appropriate model selection
        ...
        # Check if user asked for code and if response contains code blocks
        handle_code_block_extraction(query, result)
        
        # Create response_data dictionary to hold both text and image content
        response_data = {
            "response": result
        }
        
        # Process and display diagram if applicable with retry mechanism
        if is_diagram_request:
            diagram_result = process_and_display_diagram_with_retry(query, result, max_attempts=3)
            if isinstance(diagram_result, dict) and "image_data" in diagram_result:
                ...
                # Add information about retry attempts if any were made
                attempts_info = ""
                if "Generated after" in diagram_result.get("message", ""):
                    attempts_info = f" {diagram_result['message'].split('(')[1].split(')')[0]}"
                
                result += f"\n\n[Diagram generated and displayed above{attempts_info}]"
                response_data["response"] = result
                
                print(f"Diagram generated with ID: {image_id}")
            elif isinstance(diagram_result, str):
                print(f"Diagram result: {diagram_result}")
                # Add the error message to the response
                result += f"\n\n[Diagram generation failed: {diagram_result}]"
                response_data["response"] = result
        
        # Update conversation history (keep for compatibility)
        conversation_history.append(f"Q: {query}\nA: {result}")
...
def upload_file():
    if 'file' not in request.files or 'query' not in request.form:
        return jsonify({"error": "No file or query part"}), 400
    file = request.files['file']
    query = request.form['query']
    user_id = request.form.get('user_id', 'default_user')
    chat_id = request.form.get('chat_id', None)  # Support chat continuation
    ...
    # Clean up any previous files for this user
    ...
    try:
        ...
        print(f"Processing file: {file.filename} with query: {query}")
        ...
        if file_extension in ['png', 'jpg', 'jpeg']:
            ...
            # Save to image DB to get ID and hash
            image_id, image_hash = dbf.save_image_to_db(image_path, user_id, IMAGE_JSON, API_BASE_URL)
            print(f"✅ Image saved with ID: {image_id}, hash: {image_hash}")
            
            # Store image context for later use
            user_file_contexts[user_id] = {
                "context_type": "image",
                "file_path": image_path,
                "file_name": file.filename,
            ...
            print(f"✅ Image context stored for user: {user_id}")
            ...
            # Handle document - use memory-aware retrieval with model selection
            user_db = handle_document_upload(file, user_id)
            result = retrieve_and_answer_with_memory(user_db, query, user_id, session_id, chat_id)
        elif file_extension in ['xls', 'xlsx']:
            # Handle excel - use memory-aware retrieval with model selection
            user_db, error = handle_excel_upload(file, user_id)
            if error:
                return jsonify({"error": error}), 500
            result = retrieve_and_answer_with_memory(user_db, query, user_id, session_id, chat_id)
        elif file_extension == 'txt':
            # Handle text - use memory-aware retrieval with model selection
            user_db = handle_text_upload(file, user_id)
            result = retrieve_and_answer_with_memory(user_db, query, user_id, session_id, chat_id)
        elif file_extension == 'csv':
            # Handle CSV - use memory-aware retrieval with model selection
        ...
        # Validate and improve result quality
        result = validate_and_improve_response(result, query)
        
        # Update conversation history with image-aware context
        if file_extension in ['png', 'jpg', 'jpeg']:
            # For images, store metadata in conversation history
        ...
        # Save to saved_chat DB with image data
        print(f"Saving chat with image_id: {image_id}, hash: {image_hash}")
        dbf.save_chat_to_db(
            user_id=user_id, 
            user_prompt=query, 
            response=result, 
        ...
        # Process and display diagram if applicable
        if is_diagram_request:
            diagram_result = process_and_display_diagram_with_retry(query, result)
            if diagram_result:
...
def feedback():
    try:
        ...
        is_image_response = data.get('is_image_response', False)
        
        print(f"Processing feedback for user {user_id}, type: {feedback_type}")
        if is_image_response:
            print("This appears to be feedback on an image response")
                ...
                if isinstance(entry, str) and "Q:" in entry and "A:" in entry:
                    ...
                    if len(parts) > 0:
                        ...
                        if q_part.startswith("Q:"):
                            ...
                            # Check for image tag in the prompt
                            if "[IMAGE:" in user_prompt:
                                print("Found image tag in conversation history")
        ...
        # Check multiple sources to find image context
        # 1. First check user_file_contexts (most direct source)
        if user_id in user_file_contexts:
            context = user_file_contexts[user_id]
            if context.get("context_type") == "image":
                image_id = context.get("image_id")
                image_hash = context.get("image_hash")
                print(f"Found image context from user_file_contexts: id={image_id}, hash={image_hash}")
        
        # 2. Check if there's an image tag in the prompt
        if not image_id and "[IMAGE:" in user_prompt:
            try:
                image_id = user_prompt.split("[IMAGE:")[1].split("]")[0].strip()
                print(f"Extracted image_id from prompt tag: {image_id}")
            except:
                pass
                
        # 3. If this is flagged as image response but we still don't have image data
        if is_image_response and not image_id:
            print("Attempting to find image context from saved chats...")
            # Try to find the most recent saved chat with this response and image data
            try:
                ...
                if os.path.exists(output_file):
                    ...
                    # Look for a chat with this response and image data
                    for chat in reversed(saved_chats):  # Start with most recent
                        if chat.get("response") == response_text and chat.get("image_id"):
                            image_id = chat.get("image_id")
                            image_hash = chat.get("image_hash")
                            print(f"Found image context in saved chats: id={image_id}, hash={image_hash}")
                            break
            except Exception as e:
                print(f"Error searching saved chats: {e}")
        
        print(f"Final image context: id={image_id}, hash={image_hash}")
        ...
        error_msg = f"Error processing feedback: {e}"
        print(error_msg)
        return jsonify({"error": error_msg}), 500
...
@app.route('/incognito_chat', methods=['POST'])
def incognito_chat():
    data = request.json
    query = data.get('query', '')
    user_id = data.get('user_id', 'default_user')
    incognito_status = data.get('incognito_status', True)  # Whether incognito is on or off
    new_chat = data.get('new_chat', False)  # Flag to indicate if this is a new chat
    ...
    try:
        ...
        if not incognito_status:
            ...
            return jsonify({"response": "Incognito session ended. Chat history has been cleared.", 
                           "incognito": False}), 200
        ...
        return jsonify({"response": result, "incognito": True}), 200

    except Exception as e:
        error_msg = f"Error in incognito chat: {e}"
        print(error_msg)
        return jsonify({"error": error_msg, "incognito": True}), 500

@app.route('/incognito_doc', methods=['POST'])
def incognito_doc():
    file = request.files['file']
    query = request.form.get('query', '')
    user_id = request.form.get('user_id', 'default_user')
    incognito_status = request.form.get('incognito_status', 'true').lower() == 'true'
    ...
    if not incognito_status:
        flush_incognito_history(user_id)
        return jsonify({"response": "Incognito session ended. Document context has been cleared.", 
                       "incognito": False}), 200
    ...
    try:
        ...
        if loader:
            ...
            temp_db = store_vector_db(chunked_docs, embed, temp_vector_db_path, index_name)
            ...
            return jsonify({"error": "Unsupported file type", "incognito": True}), 400
    except Exception as e:
        error_msg = f"Error processing file in incognito mode: {e}"
        print(error_msg)
        return jsonify({"error": error_msg, "incognito": True}), 500

# Update new_chat route to handle incognito mode
@app.route('/new_chat', methods=['POST'])
def new_chat():
    try:
        user_id = request.json.get('user_id', 'default_user')
        incognito_status = request.json.get('incognito_status', False)
        ...
        if incognito_status:
            ...
            threading.Thread(target=clean_user_files, args=(user_id,)).start()
        ...
        return jsonify({"error": f"Error restarting chat session: {e}"}), 500
